...
Q1:
Compare between linear search and binary search algorithms by searching for the numbers 
45 and 54 in the following list: 3 8 12 34 54 85 61 110
...
Binary search searches an element in a given sorted list of length N by comparing it with the value of the pivot element with O(log(N)) time complexity. 
Since it needs a sorted list (conventionally sorted in ascending order), the given list will need to be sorted first before performing the binary search. Sorted list: 3 8 12 34 54 61 85 110
...
Binary search algorithm will take the middle element 54 as pivot point and found that it is larger than 45, so the high will be set to element 34 and the mid will be set to 8, 
then because the mid is less than 45 so the low will be set to 12 and the mid will also be 12, then the high will become smaller than low and the search returns -1 as element is not found. Number of iterations is 3.
